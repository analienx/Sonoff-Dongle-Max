...
## 4. Restore the actual Zigbee network identity, then apply and start

Before activating the P10 on the household network, use a separately reviewed, exact-version-compatible Ember→Z-Stack restore method to transfer and READ BACK the effective SONOFF coordinator IEEE (factory/secondary distinction), PAN and extended PAN IDs, channel, network key, key sequence, frame/security counters and required Trust Center state. Confirm the original SONOFF is physically isolated, no other client owns the P10 and no new network was formed. Zigbee2MQTT does not guarantee cross-stack migration without repairs. **None of the above USB/staging scripts performs this radio-state operation, so they must not auto-authorize startup.** Do not erase the source backup or fabricate missing Ember `devices` entries.

Once that radio-state operation has independently succeeded and been recorded, run the read-only plan first:

```text
py -3 deploy/p10_ha_apply.py --stage C:/Workspace/.analienx/sonoff-private/issues/p10/staged/usb-UNIQUE --phase target
```

The explicit `--execute` form additionally needs `--approval APPLY_MR4U_ONLY_AFTER_SONOFF_ISOLATION` and four `--ack` flags: `source-isolated`, `existing-network-restored`, `effective-ieee-verified`, `exclusive-radio-client`. It updates **both** YAML and add-on options while leaving Z2M stopped. Check that the new effective port is the verified by-id path, `adapter: zstack`, and unrelated settings are unchanged. Next use the separate read-only `p10_ha_start.py --stage STAGE --phase target` plan; its explicit start approval is `START_MR4U_AFTER_SONOFF_ISOLATION` and it requires the same four acknowledgements. Read the startup logs once; a started app is not proof of restored network identity. Do not bypass a mismatch by deleting database/coordinator backup or forming a new network.
