...
- key table 1 -> 12
- multicast table 26 -> 32

The linked `.bss` delta must be exactly **+728 bytes** and the memory-manager reservation must remain unchanged. Routing, RF, retry policy, concentrator delivery-failure threshold, UART baud/flow control, network identity and NVM layout remain unchanged.
...
| B2 multicast membership | **26 -> 32 accepted** as low-cost membership headroom; not described as multicast-send admission capacity. |
