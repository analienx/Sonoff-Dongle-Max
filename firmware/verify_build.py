...
def _slcp_named_blocks(text: str, name: str) -> list[str]:
    """Return complete top-level configuration entries for *name*.

    The builder has emitted equivalent SLCP condition fields in more than one
    YAML serialization.  Verification must be semantic enough to tolerate that
    formatting while remaining strict about which named entry is accepted.
    """
...
def extract_eusart_rx_buffer(text: str) -> int:
    ...
    for block in _slcp_named_blocks(text, name):
        ...
        if len(value) != 1:
            die(f"{name}: EUSART entry must contain exactly one numeric value, got {value}")
        matches.append(int(value[0]))
    if len(matches) != 1:
        die(f"{name}: expected exactly one EUSART-conditioned value, got {matches}")
    return matches[0]
