...
def stage_files(directory: Path, phase: str) -> dict:
    ...
    if state.get('target_znp_verified_from_ha') is not True:
        raise RuntimeError('No exact target Radio 2 endpoint is staged and verified')
...
def apply(directory: Path, phase: str, approval: str, acknowledgements: list[str]) -> dict:
    ...
    required = ({'source-isolated','existing-network-restored','effective-ieee-verified','exclusive-radio-client'}
...
def main(argv=None) -> int:
    ...
    p.add_argument('--ack',action='append',default=[],choices=('source-isolated','target-isolated',
                       'existing-network-restored','effective-ieee-verified','exclusive-radio-client'))
    a=p.parse_args(argv)
    try:
