...
def verify_candidate(candidate: dict, *, target_board: str,
                     min_neighbor: int, min_tclk: int) -> dict:
    ...
    for table in REQUIRED:
        ...
        if (entry.get("proof") != "exact-build" or
                ...
                not digest or entry.get("image_sha256", "").lower() != digest.lower()):
            item["reasons"].append(f"{table}: exact-image source evidence and hash linkage missing")
    if not item["reasons"]:
        item["status"] = "PHASE_A_EVIDENCE_COMPLETE_REVIEW_REQUIRED"
...
def report(doc: dict, *, target_board: str, min_neighbor: int, min_tclk: int) -> dict:
...
def main(argv=None) -> int:
    ...
    try:
        ...
        # An unqualified candidate is a nonzero result for automation.
        return 0 if any(x["status"].startswith("PHASE_A_EVIDENCE_COMPLETE")
